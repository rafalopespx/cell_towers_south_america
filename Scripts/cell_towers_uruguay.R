...
celltowers_ar <- read_csv("Data/722.csv.gz")|> 
  sf::st_as_sf(coords = c("lon", "lat"), crs = 4326)

argentina <- ne_countries(scale = 'medium', type = 'map_units', returnclass = 'sf') |>
  filter(name == 'Argentina') 

ggplot() + 
  geom_sf(data = argentina)

UMTS_ar <- celltowers_ar |> 
  filter(radio == "UMTS") |>
  st_intersection(argentina)

LTE_ar <- celltowers_ar |> 
  filter(radio == "LTE") |>
  st_intersection(argentina)

ggplot() + 
  geom_sf(data = argentina, fill = "black", color = "white", size = 0.3) + 
  geom_sf(data = UMTS_ar, shape = ".", color = "#4d88ff", alpha = 0.3) +
  geom_sf(data = LTE_ar, shape = ".", color = "#cc0000", alpha = 0.5)
